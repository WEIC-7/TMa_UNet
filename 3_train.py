...
class HMTTrainer(Trainer):
    def __init__(self, env_type, max_epochs, batch_size, device="cuda:3", val_every=1, num_gpus=2, logdir="logs/", master_ip='localhost', master_port=17750, training_script="train.py"):
        super().__init__(env_type, max_epochs, batch_size, device, val_every, num_gpus, logdir, master_ip, master_port, training_script)
        self.window_infer = SlidingWindowInferer(roi_size=roi_size,
                                        sw_batch_size=1,
                                        overlap=0.5)
        self.augmentation = augmentation
        from model_mamba.segmamba_gsc2 import SegMamba_gsc2

        self.model = SegMamba_gsc2(in_chans=1,
                        out_chans=3,
                        depths=[2,2,2,2],
    ...
    def validation_end(self, val_outputs):
        ...
        left, right = dices[0].mean(), dices[1].mean()

        print(f"dices is {left, right}")

        mean_dice = (left + right) / 2
        
        self.log("left", left, step=self.epoch)
        self.log("right", right, step=self.epoch)
...
if __name__ == "__main__":
    ...
    trainer = HMTTrainer(env_type=env,
