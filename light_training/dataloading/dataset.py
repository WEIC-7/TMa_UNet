...
# def get_train_val_test_loader_from_train(data_dir, train_rate=0.8, val_rate=0.1, test_rate=0.2, seed=42):
#     ## train all labeled data 
#     ## fold denote the validation data in training data
#     all_paths = glob.glob(f"{data_dir}/*.npz")
#     # fold_data = get_kfold_data(all_paths, 5)[fold]
...
def get_train_val_test_loader_from_train(data_dir, train_number = 40, val_number = 10,test_number=8):
    ## train all labeled data 
    ## fold denote the validation data in training data
    all_paths = glob.glob(f"{data_dir}/*.npz")
    # fold_data = get_kfold_data(all_paths, 5)[fold]
